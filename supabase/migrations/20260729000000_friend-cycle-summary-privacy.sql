...
-- events, but never raw HealthKit metadata, flow intensity, symptoms, or signal
-- samples (BBT, cervical mucus, ovulation/progesterone tests).

-- Backfill automatic sharing for accepted friendships created before the
-- accept-time helper was introduced. Removing a friend deletes the accepted
-- request, so removed relationships are not restored here.
...
-- Raw snapshots and raw event payloads are owner-only. Friends use the
-- security-definer projection functions below instead of selecting tables.
          ...
          (
            select jsonb_agg(
              jsonb_build_object(
                ...
                -- "unspecified" preserves period-day presence without exposing intensity.
                'flowValue', 1,
