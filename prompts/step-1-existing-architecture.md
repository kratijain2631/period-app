...
3. Describe the intended data flow end-to-end: inputs (Apple Health cycle data), processing (state containers, native HealthKit bridges, Supabase syncing, background tasks), outputs (UI updates, notifications, analytics) with explicit pointers to the files/modules that should exist.
...
5. Highlight foundational constraints the net-new implementation must respect from the outset: HealthKit permission flows, offline support, privacy, localization, accessibility, testing requirements.
...
- Reference concrete files/paths that should be created; when the file does not yet exist, describe the recommended structure
