...
### Fixed
- **The Apple Health permission sheet no longer says we'll "update" your health data.** The app is read-only — it never writes to Apple Health — but iOS was showing the "…would like to update your health data" framing because the HealthKit plugin always added the write-usage key. Patched the plugin to omit it, so the prompt now only asks to *read*. _(Needs on-device verification once built; read access + background delivery are unchanged.)_ (`patches/@kingstinct+react-native-healthkit+10.1.0.patch`, `app.config.ts`.)
