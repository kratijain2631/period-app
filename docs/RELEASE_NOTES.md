...
### Docs / process
- **Logged 6 pre-existing `tsc` type errors (2026-07-30).** `npx tsc --noEmit` is red (SDK type-version drift in `usePushNotifications`, `appleAuth`, `sessionStore`, `FeedScreen`) even though the app bundles fine. Recorded in BUGS.md so `tsc` can be cleaned up and used as a CI gate.
