...
### Fixed
- **The feed is now friends-only — no more seeing strangers' posts.** Post visibility was open to every signed-in user (`posts_select_all`), so a new account saw everyone's posts. A migration replaces it with a policy that only exposes your own posts and those of mutually-sharing friends (matching how cycle events/snapshots already work). Enforced server-side — takes effect on applying the migration, no rebuild. (Apply `supabase/migrations/20260724193000_posts-friends-only-select.sql`.)
