...
- **Read Apple Health's cycle data and display it** — a cycle wheel/calendar and a clear "which phase you're in" (§ Cycle). _(Note: Apple's predictions aren't exposed to us — we compute our own phase model from the raw data. See [CYCLE_SYNC.md](CYCLE_SYNC.md).)_
...
| **Relative/competitive hook** | Beli ranks restaurants; Strava segments/leaderboards | ✅ **Sync-score leaderboard** (built 2026-08-02) — a top-5 "Most in sync 🔴" ranked list on the Circle screen. |
...
> **⚠️ Reality update (2026-08-02): we build our own model, because Apple's predictions aren't available.** The original plan below said "lean on Apple's predictions, don't build our own model." That turned out to be impossible: **HealthKit does not expose Apple's cycle predictions** to third-party apps (there is no prediction API — only the raw logged samples). So the app now computes its **own** phase model from your menstrual-flow data + optional signals (ovulation tests, cervical mucus, progesterone, BBT), with an `observed` vs `estimated` source flag, real cycle-length-based phase windows, real-transition dating, and a predicted next transition. **The full system — phase derivation, sync triggers, posting, and every notification/background mechanism — is documented in [CYCLE_SYNC.md](CYCLE_SYNC.md).**
>
> Remaining near-term cycle work:
> 1. **Richer display** — the cycle wheel now reflects your real cycle length; extend to a full **calendar / history** view of past cycles.
> 2. **Read more HealthKit types** — symptoms, etc., to enrich the display and improve the `observed` phase.
> 3. **In-app logging + write-back** — let users log in *our* app and write to Apple Health (see [TODO.md](TODO.md)); this is also the unlock for reliable Strava-style auto-posting (see [CYCLE_SYNC.md](CYCLE_SYNC.md) §5).
>
> *(Original deferred note kept for history: building our own model was meant to be "Later" — but it's now the actual approach, since Apple gives us no predictions to lean on.)*
...
- [~] **Find friends from contacts.** _(Code complete 2026-08-12; migration/native-build/on-device verification pending.)_ Your Circle now requests Contacts access only after a user taps the feature, supports iOS limited/full access, matches normalized email hashes instead of uploading raw address-book emails, and keeps “read my contacts” separate from the default-off “let contacts find me” setting. Phone matching remains a follow-up once accounts support verified phone numbers.
- **Add a verified phone number to your account + find contacts by phone.** Let users optionally add and verify a phone number (OTP), edit/remove it from account settings, and use normalized phone-number hashes alongside email hashes for contact discovery. This closes the Apple “Hide My Email” matching gap. Keep phone discoverability controlled by the existing default-off **“Let contacts find me”** preference; never upload raw address-book phone numbers, never expose someone’s phone number in search results, and support country-aware E.164 normalization.
...
- [x] **Sync-score leaderboard — your top friends (signature feature).** _(Done 2026-08-02: a top-5 "Most in sync 🔴" ranked list now renders on the Circle screen above the friends list — medal badges for the top 3, each friend's phase avatar + score %, tappable to their sync page. Shows when you have ≥2 scored friends. `FriendsScreen.tsx`.)_ Rank your friends by sync score — the Beli-style *relative* hook. **Follow-ups:** "you're most in sync with X" moments + monthly "who you synced with most" recap. See PITCH → "sync scores are the signature hook."
...
> **Notifications — current state (audited 2026-08-05).** Three layers, distinct status. Per-build detail lives in [RELEASE_NOTES.md](RELEASE_NOTES.md); this is the one-place summary.
>
> | Layer | What it does | Status | Where |
> |---|---|---|---|
> | **Client receive-side** (`usePushNotifications`, mounted in `App.tsx`) | Requests permission, gets an Expo push token, registers it to `device_tokens`, handles taps (deep-link nav) | ✅ **Shipped.** Needs on-device confirm that tokens actually land in `device_tokens` | RELEASE_NOTES "2026-07-21 baseline" |
> | **Local (on-device) nudges** (`localCycleNotifications.ts`) | (a) scheduled reminder at 10am on the predicted next-phase day; (b) immediate local notification on a background-detected phase change. **No server needed** — this is the part that actually notifies you today | ✅ **Shipped (build 1.0.8).** ⏳ Pending on-device verification (scheduled/background firing can't be tested off-device) | RELEASE_NOTES 1.0.8 |
> | **Server-driven push** (`notifications-handler`, `phase-change-reminder`) | Lock-screen push while the app is fully closed, and pushing friends about your change | ❌ **Not live.** `notifications-handler` undeployed (needs an Expo access token); `phase-change-reminder`'s cron **runs daily and succeeds** (vault secrets confirmed present 2026-09-07). `notifications-handler` still undeployed (needs an Expo access token); and `cycle-guidance`/`friend-recommendations` crons were never scheduled (their `schedule-*` migrations weren't applied) | [TODO.md](TODO.md) "Fix avatars + AI features", RELEASE_NOTES 1.0.8 |
>
> **Net:** the app notifies you **locally on your own device** around cycle changes; it does **not** yet push you (or friends) from the server. Unblocking the server layer = deploy `notifications-handler` with an Expo access token **+** set the two vault secrets. On-device verification of the shipped layers is tracked in the [TODO.md](TODO.md) checklist.

- [x] **Dedicated notifications page with a new/read distinction (requested 2026-07-30, owner).** _(Built 2026-07-30 — code complete, tsc-green; needs the migration applied + on-device verification.)_ The bell now **navigates to a full-screen `NotificationsScreen`** (in the Home stack) instead of opening the bottom sheet. The page shows **Friend requests** (actionable), a **New** section (what was unread when you opened), and **Earlier** (already read), with unread rows accented (dot + accent border) and read rows dimmed. Opening the page **marks everything read but keeps it visible** (no more "clears the moment you open it"), and the bell badge now counts **only unread**. Read state: DB notifications use a new **`read_at` column** (per owner's choice — migration `20260730120000_notifications-read-at.sql`, which also adds the missing `notifications_update_own` UPDATE policy); "X accepted your request" items persist as *seen* (existing AsyncStorage pattern) so they don't re-alert. Files: `notifications.ts` (`read_at` + `markNotificationsRead`), `useNotifications.ts` (`markAllRead`, unread-only count), new `app/features/notifications/screens/NotificationsScreen.tsx`, `AppNavigator.tsx`, `HomeScreen.tsx` (bell → navigate). **Caveats:** (1) the **migration must be applied to Supabase** (SQL editor) for DB read-state to persist — until then `markNotificationsRead` no-ops gracefully; (2) the DB `notifications` table is still **not populated** (no trigger / the `notifications-handler` isn't deployed), so today the page's content is friend requests + acceptances — pairs with the push/realtime work below; (3) acceptance "history" isn't deeply persisted (a read acceptance drops off on a later reload, since it's derived from the unseen set). The old `NotificationsSheet` stays (still used by the legacy `FeedScreen`).
