...
> | **Server-driven push** (`notifications-handler`, `phase-change-reminder`) | Lock-screen push while the app is fully closed, and pushing friends about your change | ❌ **Not live.** `notifications-handler` undeployed (needs an Expo access token); `phase-change-reminder` deployed but its cron can't run — the `project_url` / `publishable_key` **vault secrets don't exist** (this also silently breaks every other cron) | [TODO.md](TODO.md) "Fix avatars + AI features", RELEASE_NOTES 1.0.8 |
>
