...
- **Friend-cycle migration mismatch (2026-08-06):** `20260729000000_friend-cycle-summary-privacy.sql` was applied live but never committed to `main`, while the client still selected raw cycle tables. The source/client port is now restored and uses only sanitized RPCs. `20260806000000_legacy-friend-cycle-read-compat.sql` was briefly applied, but the owner confirmed this is dev-only and all testers can update; run `20260806010000_remove-legacy-friend-cycle-read.sql` in the SQL editor to restore owner-only raw-table policies. Do not rerun earlier migrations or use `db push`.

## Build & submission gotchas

- **Contact discovery requires both a database migration and a fresh native build (added 2026-08-12).** Apply `supabase/migrations/20260812090000_contact-friend-discovery.sql` manually in the Supabase SQL editor before testing; it additively creates `users.discoverable_by_contacts` (default `false`) and the `contact_friend_matches` RPC. Then create/reinstall a native dev or TestFlight build because `expo-contacts` and `NSContactsUsageDescription` are native configuration — a Metro reload cannot add them to an older binary. On iOS 18+, test both **Limited Access** (selected contacts only, including “Choose more contacts”) and **Full Access**, plus denial → Settings recovery. These are deliberately separate controls: the Apple permission determines which address-book entries this device can read, while the in-app “Let contacts find me” toggle determines whether this account can appear in another user’s hashed lookup. Matching currently uses account email only; Sign in with Apple private-relay emails may not match until verified phone-number support is added.
