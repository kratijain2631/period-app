...
- [ ] **Share the 1.0.3 tester link** and confirm with a few testers (incl. someone with lots of Apple Health cycle data) that it's stable. Public TestFlight join link: **https://testflight.apple.com/join/akMkE8kW** (also wired into the in-app "Invite Friends" share message — constant `TESTFLIGHT_INVITE_URL` in `app/config/branding.ts`).
