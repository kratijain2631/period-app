...
- [ ] **Apply `20260806010000_remove-legacy-friend-cycle-read.sql` in Supabase** (SQL editor) and confirm the `friend_cycle_summaries()` RPC is live. 1.0.11 reads friends **only** via that RPC (no fallback), so it must exist; applying the removal migration finishes locking raw `cycle_snapshots`/`cycle_events` to owner-only. ⚠️ Applying it **breaks friend-cycle reads for anyone still on 1.0.10 or older** — treat 1.0.11 as the required minimum.
- [ ] **(Only if needed later) Add `periodLengthDays` (+ `nextPhaseStart`/`nextPhase`) to the `friend_cycle_summaries()` projection.** Verified 2026-08-06: **no friend-facing view currently consumes these** — friend rings are the fixed-28-day `SyncRings`/`PhaseAvatar`, and `deriveCycleDay` only uses `latestSampleStart`+`cycleLengthDays`. The projection is intentionally minimal for privacy. Only extend it if a real per-friend cycle-ring (segment-sized like the Home/Profile ring) is ever built — otherwise it's dead data.
