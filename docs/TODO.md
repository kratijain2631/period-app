...
## 🎯 Next up — make cycle-guidance on-demand + always-on fallback (retire the cron as the only path)
- [ ] **Redesign how the "You" page do's/don'ts (cycle guidance) get generated.** Fixes the "No guidance yet" bug ([BUGS.md](BUGS.md)) at the design level, not just by babysitting a cron.
  - **Why it's a cron today (the rationale, so we don't "fix" it into something worse):** it's a **precompute-and-cache** pattern — the AI call is slow (seconds) and costs money per call, and do's/don'ts only change **per phase**, so a nightly job generates one `cycle_guidance` row and the app reads it instantly + pays once, instead of calling the model on every page open.
  - **Why that's fragile (what bit us):** the cron is a **single point of failure** — *not scheduled → empty forever* (its schedule migration was never applied), and even once scheduled the function currently **500s** (`permission denied for table cycle_snapshots` — its service-role key isn't in effect; see the separate blocker in [BUGS.md](BUGS.md) / "Fix avatars + AI" below). Both failures are invisible because `net.http_post` only reports the request was *dispatched*, never the function's status.
  - **The better design (do both):**
    1. **Client-side fallback (quick win, kills the empty state immediately):** render the static `fallbackRecommendations` (already in the code, referenced in [FEATURES.md](FEATURES.md)) whenever there's no AI row yet, so the card is **never** empty regardless of crons/function health. `ProfileScreen.tsx` (`loadCycleGuidance`, `guidanceEmptyLabel`, `phaseDos`/`phaseDonts`).
    2. **Lazy on-demand + cache (removes the cron as a SPOF):** generate guidance the **first time a user views a phase**, write the `cycle_guidance` row, and reuse it until their phase changes. Cheap (once per user per phase), self-healing (no scheduling to forget), instant thereafter. The nightly cron then becomes an **optional pre-warm**, not the only path.
  - **Net:** (1) alone makes "No guidance yet" disappear; (2) drops the cron dependency entirely. Files: `ProfileScreen.tsx`, `app/services/supabase/*` (`fetchCycleGuidance`), `supabase/functions/cycle-guidance`. _(Independent of, but related to, the service-role-key 500 — the fallback means the page works even while that's unresolved.)_
...
## Friend-cycle privacy RPC (1.0.11) — follow-ups
- [x] **Raw-table lockdown is LIVE (confirmed 2026-08-07 via `pg_policies`).** `cycle_snapshots`/`cycle_events` have only `*_select_own` policies — the `20260806010000` end-state is already in effect (no `*_select_own_or_friends` compat policy present). Friends read only via `friend_cycle_summaries()` / `shared_cycle_events()`. _(This corrects an earlier stale note that treated the removal migration as pending.)_
- [ ] **Get every tester off builds ≤1.0.10 (required minimum: 1.0.11).** The lockdown above **breaks friend-cycle sync on ≤1.0.10** only (those builds read the raw tables → now empty); that's the sole *breaking* reason to upgrade. **1.0.11 is the floor** — anyone on it is fine. **1.0.12 (build 33) is the latest** and is **additive/non-breaking** vs 1.0.11 (it just adds contact discovery), so moving 1.0.11→1.0.12 is only needed to test/use contacts, not for correctness. First check who's actually on ≤1.0.10 (TestFlight) — if everyone's already ≥1.0.11 this is a non-issue. Also confirm the RPCs exist (1.0.11 has no fallback): `select proname from pg_proc where proname in ('friend_cycle_summaries','shared_cycle_events');`
- [ ] **(Future only) If a real per-friend cycle-ring is ever added, extend `friend_cycle_summaries()` with `periodLengthDays` (+ `nextPhaseStart`/`nextPhase`).**
  - **Symptom that means you need this:** a *friend's* cycle ring draws its period (menstruation) segment at the default 5 days instead of their real length. (Can't happen today — friend views use the fixed-28-day `SyncRings`/`PhaseAvatar`, never the segment-sized `CycleRing`.)
  - **Why omitted now:** verified 2026-08-06 that no friend view reads these (`deriveCycleDay` uses only `latestSampleStart`+`cycleLengthDays`; `computeSyncScore` uses neither). Minimal projection = better privacy.
  - **Exact fix (~10 min):** add a **new** additive migration (never edit an already-applied one) doing `create or replace function public.friend_cycle_summaries()` with `'periodLengthDays', cs.snapshot -> 'periodLengthDays'` (and `nextPhaseStart`/`nextPhase`) added to the `jsonb_build_object`, mirroring `supabase/migrations/20260729000000_friend-cycle-summary-privacy.sql`; apply it in the Supabase SQL editor. **No client change needed** — `CycleSnapshot` already types these fields, so they flow through automatically once the RPC returns them; then pass `periodLengthDays={friendSnapshot.periodLengthDays}` to the new friend `CycleRing`.
...
- [~] **Activate server-side phase-change reminders (built 2026-08-02).** Progress: ✅ migration `20260802120000_phase-change-reminders.sql` **applied** (owner, 2026-08-02); ✅ `phase-change-reminder` function **deployed** (2026-08-02). **Remaining blockers:** **(1) ✅ Vault secrets present (confirmed 2026-09-07).** `project_url` + `publishable_key` exist in Vault with correct values — the earlier "they don't exist / `null value in column url`" blocker is **resolved** (an even earlier note claimed they were missing; they aren't). **Confirmed 2026-09-07 via `cron.job` + `cron.job_run_details`:** the `daily-phase-change-reminder` cron exists and **succeeds daily** (since 2026-08-19). But it is the **only** scheduled job — `cycle-guidance` and `friend-recommendations` have **no cron job**, i.e. their schedule migrations were never applied. **Fix (apply in SQL editor):** `20260124191340_schedule_cycle_guidance.sql` (creates `nightly-cycle-guidance`, 03:00 UTC) and `20260215193000_schedule-friend-recommendations.sql`. Both read the now-present vault secrets, so they should run once scheduled. _(Caveat: cron `status=succeeded`/`"1 row"` only means the async `net.http_post` was **dispatched** — it does not confirm the edge function returned 200. If guidance still doesn't populate after scheduling, the function may be 401ing on the `sb_publishable_` bearer → redeploy the function with `--no-verify-jwt`, and check `net._http_response` for the actual status.)_ **(2) Verify push tokens register** on a real device on build ≥1.0.8 (`device_tokens` table has rows; `usePushNotifications` runs with notifications allowed). Also only reaches users whose uploaded snapshot has `nextPhaseStart` (build ≥1.0.8).
