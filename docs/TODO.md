...
- [ ] **Background posting of cycle updates (requested 2026-07-30, owner — do next).** Proper solution for the "(b) background delivery" half of the phase-post bug ([BUGS.md](BUGS.md)): auto-post phase-transition / period events **while the app is closed**, not only on next open. Today `syncHealthData` runs on app launch/foreground (`useCycleSyncLifecycle`) + a best-effort `backgroundSync.ts` task that iOS rarely wakes. Approach to scope with owner: harden the `expo-task-manager` / `expo-background-fetch` background task (verify `defineTask` at module scope, `BackgroundFetch.registerTaskAsync`, minimum-interval, `startOnBoot`/`stopOnTerminate`) and/or a server-side scheduled path — note HealthKit is on-device, so a server can't read it; delivery of *lock-screen* notifications still needs the `notifications-handler` + an **Expo push token**. Needs an on-device verification pass (background execution can't be tested in the constrained CLI env).
