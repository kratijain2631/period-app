...
- [ ] **Beta 4 (1.0.2) TestFlight submit — BLOCKED on `ascAppId`.** `eas submit --non-interactive` (and `--auto-submit`) need `submit.production.ascAppId` in `eas.json`, which is empty; without it EAS falls back to an interactive **Apple ID login** ("ensure app exists" step) that can't run headless. Fix once and for all: add the app's App Store Connect **Apple ID** number (App Store Connect → the app → **App Information → General Information → Apple ID**) to `eas.json` `submit.production.ascAppId`, then `eas submit --platform ios --profile production --latest`. (Prior betas submitted interactively with a cached Apple session, so this never surfaced — see [TROUBLESHOOTING.md](TROUBLESHOOTING.md).)
