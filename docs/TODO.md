...
- [~] **Fix the consent model (highest priority — privacy honesty for menstrual data).** 🚧 (WIP — Codex, 2026-07-29) Implemented and tested in code: accepting a friend request is explicitly the mutual consent action; friends receive only a safe phase/calendar projection; raw snapshots/events are owner-only; intentionally published posts, moods, and safe cycle updates remain visible. Removed the refresh-time sharing reset and obsolete revoke API. `20260729000000_friend-cycle-summary-privacy.sql` was applied successfully through the Supabase SQL Editor on 2026-07-29. **Remaining:** verify with two friend accounts before marking done.
