...
- [ ] **Activate server-side phase-change reminders (built 2026-08-02).** Three steps: (1) **apply** migration `20260802120000_phase-change-reminders.sql` (adds `cycle_snapshots.last_reminded_phase_start`, the `due_phase_reminders()` / `mark_phase_reminded()` functions, and the daily `daily-phase-change-reminder` cron — needs vault secrets `project_url` + `publishable_key`, same as the other crons); (2) **deploy** the function: `npx supabase functions deploy phase-change-reminder --project-ref mmomoyszozclfjdvjbeb` (add `--no-verify-jwt` if the cron's bearer isn't accepted); (3) **verify push tokens register** — needs a build where `usePushNotifications` runs on a real device with notifications allowed (check the `device_tokens` table has rows). No Expo access token required (Expo push works unauthenticated), but set `EXPO_PUSH_ACCESS_TOKEN` as a function secret if you enable Expo Enhanced Security. Note: only reaches users whose uploaded snapshot has `nextPhaseStart` (i.e. after they run a build ≥1.0.8).
