...
- [ ] **Get all testers onto 1.0.11 (build 32) — it is the required minimum build.** The lockdown above **breaks friend-cycle sync on ≤1.0.10** (those builds read raw tables → now empty). This is a **deliberate exception to [INSTRUCTIONS §12](INSTRUCTIONS.md)** (backward-compat), acceptable only because closed-beta testers update together; it fully resolves once everyone is on 1.0.11. Also confirm the RPCs exist (1.0.11 has no fallback, so they must): `select proname from pg_proc where proname in ('friend_cycle_summaries','shared_cycle_events');`
