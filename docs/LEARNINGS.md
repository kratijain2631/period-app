...
- **Mutual-consent sharing (schema intent — ⚠️ not how the app currently behaves):** the *design* is that a friend sees your cycle data only if `has_shared = true` on **both** sides (`friend_sharing` table, default `false`), with `FriendSyncScreen` showing a "Consent Needed" state until consent is mutual — stronger than most apps and good for App Review. **In practice this isn't what happens:** `ensure_friend_sharing` auto-sets `has_shared = true` both ways on friend-accept (and `loadFriends` re-runs it every load), and there's no revoke UI, so accepting a friend = instant two-way cycle-data sharing with no explicit opt-in. The RLS gate itself is correct; the *consent step* is bypassed. Reconcile before leaning on this claim for App Review — see [BUGS.md](BUGS.md) (consent-model + dead-toggle) and [TODO.md](TODO.md) (Code health → fix the consent model).
