...
- **Friend acceptance is mutual cycle-sharing consent (decision 2026-07-29).** The point of becoming friends in this app is to share core cycle context, so accepting a request creates both `friend_sharing` rows. The safe boundary is enforced server-side: friends receive phase, cycle/calendar timing, date-only menstrual samples needed for sync scores, and sync freshness—not raw HealthKit metadata, flow intensity, symptoms, BBT, cervical mucus, or ovulation/progesterone signals. Posts and selected moods are visible when deliberately published. Removing the friend ends access. `friend_cycle_summaries()` / `shared_cycle_events()` provide the safe projections; raw table rows are owner-only.
