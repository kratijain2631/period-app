...
- **Mutual-consent sharing:** a friend sees your cycle data only if `has_shared = true` on **both** sides (`friend_sharing` table, default `false`). `FriendSyncScreen` shows a "Consent Needed" state until consent is mutual. This is stronger than most apps and is good for App Review.
