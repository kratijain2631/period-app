...
7. **Keep the machine awake for unsupervised/long runs.** When working while the owner is away (oneshot, or any long EAS build/submit), start `caffeinate` in the background first (e.g. `nohup caffeinate -dimsu &`) so the laptop doesn't sleep and stall an upload mid-flight — the owner's expectation is that the work is *done* by the time they're back.
8. **Verify the build actually registered — don't trust "launched".** A background `eas build` can hang at "Compressing project files / uploading" and never reach EAS (an earlier run silently died there, so a version bump shipped *no* build). After kicking a build, confirm it appears in `eas build:list --platform ios --limit 1` with the new version/build number and a non-failed status before reporting it as shipped; if it hung, kill the stale process and re-run.
